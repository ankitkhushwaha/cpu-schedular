...
void init_rr() {
    ...
    process_core->scheduler_core = schedular_rr;
    process_core->wakeUp_core = wakeUp_rr;
    process_core->add_arrival_core = add_arrival_process_rr;
    return;
error:
    exit(EXIT_FAILURE);
}

static bool _simulate_rr() {
...
void *add_arrival_process_rr(burst_data **data) {
    int j = 0;
...
void *schedular_rr() {
    ...
    while (1) {
        ...
        if (sem_trywait(&ready_count) == 0) {
            ...
            running_pd = remove_from_readyQueue_rr();
            if (running_pd) {
...
void *wakeUp_rr() {
    ...
    while (atomic_load(&TERMINATED_PROCESS) < TOTAL_PROCESS) {
        ...
        if (sem_trywait(&wait_count) == 0) {
            sleeping_pd = remove_from_waitQueue_rr();
            if (sleeping_pd) {
