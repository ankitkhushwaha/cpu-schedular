...
void *add_arrival_process_fifo(burst_data **data) {
    ...
    while (j != t_process) {
        if ((*data)->b_data[j]->a_time <= read_global_counter()) {
            ...
            add_to_readyQueue_fifo(p);
            add_to_taskList_fifo(p);
...
void *schedular_fifo() {
    ...
    while (1) {
        ...
        if (sem_trywait(&ready_count) == 0) {
            ...
            running_pd = remove_from_readyQueue_fifo();
            if (running_pd) {
...
void *wakeUp_fifo() {
    ...
    while (atomic_load(&TERMINATED_PROCESS) < TOTAL_PROCESS) {
        ...
        if (sem_trywait(&wait_count) == 0) {
            // debug("TERMINATED_PROCESS: %d, TOTAL_PROCESS: %d, index: %d",
            sleeping_pd = remove_from_waitQueue_fifo();
            if (sleeping_pd) {
